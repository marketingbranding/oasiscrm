...
@section('title', 'Edit Changelog - Oasis CRM')

@section('content')
    <div class="bg-[#8c9ae0] border-2 border-black px-4 py-2 mb-6">
        <h1 class="font-['Arial_Black'] font-black text-xl uppercase">Edit Changelog</h1>
    </div>

    <div class="border-2 border-black bg-white p-5 max-w-2xl">
        <form method="POST" action="{{ route('changelogs.update', $changelog) }}">
            @csrf @method('PUT')
            <div class="mb-4">
                ...
                <select name="category" class="w-full border-2 border-black px-3 py-2 text-sm font-['Times_New_Roman'] bg-white rounded-none">
                    <option value="added" {{ old('category', $changelog->category) === 'added' ? 'selected' : '' }}>ADDED — Fitur baru</option>
                    <option value="fixed" {{ old('category', $changelog->category) === 'fixed' ? 'selected' : '' }}>FIXED — Perbaikan bug</option>
                    <option value="changed" {{ old('category', $changelog->category) === 'changed' ? 'selected' : '' }}>CHANGED — Perubahan</option>
                    <option value="removed" {{ old('category', $changelog->category) === 'removed' ? 'selected' : '' }}>REMOVED — Dihapus</option>
                </select>
            ...
            <div class="mb-4">
                ...
                <input name="title" value="{{ old('title', $changelog->title) }}" class="w-full border-2 border-black px-3 py-2 text-sm font-['Times_New_Roman'] rounded-none">
            ...
            <div class="mb-4">
                ...
                <input name="version" value="{{ old('version', $changelog->version) }}" placeholder="1.0.0" class="w-full border-2 border-black px-3 py-2 text-sm font-['Times_New_Roman'] rounded-none">
            ...
            <div class="mb-4">
                ...
                <textarea name="description" rows="4" class="w-full border-2 border-black px-3 py-2 text-sm font-['Times_New_Roman'] rounded-none">{{ old('description', $changelog->description) }}</textarea>
