...
<form method="POST" action="{{ route('feedback-reports.review', $report) }}" class="border-2 border-black bg-white p-4 space-y-3 h-fit">@csrf @method('PATCH')
    <label class="block font-bold text-xs">Status<select name="status" class="mt-1 w-full border-2 border-black px-2 py-1 bg-white">@foreach($allowedStatuses as $status)<option value="{{ $status }}" @selected($report->status === $status)>{{ \App\Models\FeedbackReport::statusLabelFor($status) }}</option>@endforeach</select></label>
