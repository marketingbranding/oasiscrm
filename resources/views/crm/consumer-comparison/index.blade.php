...
@if($result)
    ...
    <section class="mb-4 border-2 border-black bg-white p-4" aria-labelledby="identity-audit-heading">
        ...
        <p class="mb-3 text-sm text-gray-700">Audit baca-saja. Tidak membuat atau mengubah identity mapping.</p><p class="mb-3 text-xs text-gray-600">Legacy comparison contains 99 collapsed kavling records, while identity audit contains 109 raw consumer rows. The 10-row difference comes from repeated/reused kavling records.</p>
        ...
        <div class="mt-3 text-sm"><strong>Proyek baris legacy bermasalah</strong>: ambigu {{ $audit['legacy']['identity_issues']['project_ambiguous']['count'] }} @foreach($audit['legacy']['identity_issues']['project_ambiguous']['rows'] as $issueRow) · {{ $issueRow['kavling'] }} @endforeach — tidak ditemukan {{ $audit['legacy']['identity_issues']['project_not_found']['count'] }} @foreach($audit['legacy']['identity_issues']['project_not_found']['rows'] as $issueRow) · {{ $issueRow['kavling'] }} @endforeach</div>
        ...
        <div class="mt-3 text-sm"><strong>Semantic enrichment coverage</strong>: at least one {{ $audit['local']['semantic_coverage']['at_least_one'] }} · all three {{ $audit['local']['semantic_coverage']['all_three'] }} · none {{ $audit['local']['semantic_coverage']['none'] }}</div>
         <div class="mt-3 grid gap-3 text-sm md:grid-cols-2"><div><strong>Raw phone+kavling diagnostics</strong><br>Legacy rows {{ $audit['diagnostics']['phone_kavling']['legacy_rows'] }} · Local applications {{ $audit['diagnostics']['phone_kavling']['local_applications'] }}<br>Raw exact matches {{ $audit['diagnostics']['phone_kavling']['raw_exact_matches'] }} · Raw unique one-to-one {{ $audit['diagnostics']['phone_kavling']['raw_unique_one_to_one'] }}<br>Duplicate legacy keys {{ $audit['diagnostics']['phone_kavling']['duplicate_legacy_keys'] }} · Duplicate local keys {{ $audit['diagnostics']['phone_kavling']['duplicate_local_keys'] }}<br>Ambiguous {{ $audit['diagnostics']['phone_kavling']['ambiguous_matches'] }} · Unmatched legacy {{ $audit['diagnostics']['phone_kavling']['unmatched_legacy'] }} · Unmatched local {{ $audit['diagnostics']['phone_kavling']['unmatched_local'] }}<br>Final UNIQUE_PHONE_KAVLING {{ $audit['candidates']['UNIQUE_PHONE_KAVING'] ?? $audit['candidates']['UNIQUE_PHONE_KAVLING'] }}</div><div><strong>NIK fingerprint diagnostics</strong><br>Legacy valid {{ $audit['diagnostics']['nik']['legacy_valid'] }} · unique fingerprints {{ $audit['diagnostics']['nik']['unique_legacy_fingerprints'] }} · duplicate groups {{ $audit['diagnostics']['nik']['duplicate_legacy_groups'] }}<br>Local valid {{ $audit['diagnostics']['nik']['local_valid'] }} · unique fingerprints {{ $audit['diagnostics']['nik']['unique_local_fingerprints'] }} · duplicate groups {{ $audit['diagnostics']['nik']['duplicate_local_groups'] }}<br>Exact matches {{ $audit['diagnostics']['nik']['exact_matches'] }} · SAFE_NIK_ONE_TO_ONE {{ $audit['diagnostics']['nik']['safe_one_to_one'] }} · SAFE_NIK_LINKABLE {{ $audit['diagnostics']['nik']['safe_linkable'] }}<br>Blocking conflict among one-to-one candidates {{ $audit['diagnostics']['nik']['identity_conflicts'] }} · TRUE_CONFLICT classifications {{ $audit['diagnostics']['nik']['classifications']['TRUE_CONFLICT'] }} · {{ collect($audit['diagnostics']['nik']['classifications'])->map(fn ($v, $k) => $k.': '.$v)->implode(' · ') }}<br>Legacy→multiple local {{ $audit['diagnostics']['nik']['one_legacy_multiple_local'] }} · Multiple legacy→one local {{ $audit['diagnostics']['nik']['multiple_legacy_one_local'] }}<br>Unmatched legacy {{ $audit['diagnostics']['nik']['unmatched_legacy'] }} · Unmatched local {{ $audit['diagnostics']['nik']['unmatched_local'] }}</div></div>
         <div class="mt-3 border-t-2 border-black pt-3 text-sm"><strong>Identity Bridge Plan Preview — read-only</strong><br>READY_EXISTING_SAME_APP {{ $audit['bridgePlan']['counts']['READY_EXISTING_SAME_APP'] }} · BLOCK_TRUE_CONFLICT {{ $audit['bridgePlan']['counts']['BLOCK_TRUE_CONFLICT'] }} · BLOCK_AMBIGUOUS {{ $audit['bridgePlan']['counts']['BLOCK_AMBIGUOUS'] }} · BLOCK_UNMATCHED {{ $audit['bridgePlan']['counts']['BLOCK_UNMATCHED'] }}<br>Simulated matched {{ $audit['bridgePlan']['simulation']['matched'] }} · Legacy only {{ $audit['bridgePlan']['simulation']['legacy_only'] }} · Local only {{ $audit['bridgePlan']['simulation']['local_only'] }} · Ambiguous {{ $audit['bridgePlan']['simulation']['ambiguous'] }} · Coverage {{ $audit['bridgePlan']['simulation']['coverage_percent'] }}%<br>Future write strategy: {{ $audit['bridgePlan']['future_write_strategy'] }} — {{ $audit['bridgePlan']['future_write_reason'] }}</div>
         <div class="mt-3 overflow-x-auto"><table class="min-w-full text-left text-xs"><thead class="bg-black text-white"><tr><th class="p-2">Action</th><th class="p-2">Legacy</th><th class="p-2">Local candidates</th><th class="p-2">Reason</th><th class="p-2">Duplicate NIK</th></tr></thead><tbody>@foreach($audit['bridgePlan']['rows'] as $row) @if($row['action'] !== 'READY_EXISTING_SAME_APP')<tr class="border-b border-gray-300"><td class="p-2 font-bold">{{ $row['action'] }}</td><td class="p-2">{{ $row['legacy_name'] }} · {{ $row['legacy_phone'] }} · {{ $row['legacy_kavling'] }}</td><td class="p-2">{{ implode(', ', $row['local_applications']) }} · {{ implode(', ', $row['local_names']) }} · {{ implode(', ', $row['local_kavlings']) }}</td><td class="p-2">{{ $row['reason'] }}</td><td class="p-2">{{ $row['duplicate_nik'] ? 'yes' : 'no' }}</td></tr>@endif @endforeach</tbody></table></div>
        ...
        <div class="mt-3 overflow-x-auto"><table class="min-w-full text-left text-xs"><thead class="bg-black text-white"><tr><th class="p-2">Nama</th><th class="p-2">Phone</th><th class="p-2">Kavling</th><th class="p-2">Consumer status</th><th class="p-2">Kategori</th><th class="p-2">Detail</th></tr></thead><tbody>@foreach($audit['candidates']['rows'] as $row)<tr class="border-b border-gray-300"><td class="p-2">{{ $row['name'] ?: '—' }}</td><td class="p-2">{{ $row['phone'] }}</td><td class="p-2">{{ $row['kavling'] ?: '—' }}</td><td class="p-2">{{ $row['consumer_status'] }}</td><td class="p-2 font-bold">{{ $row['category'] }}</td><td class="p-2">{{ $row['reason'] ?: '—' }} · L{{ $row['legacy_candidate_count'] }}/L{{ $row['local_candidate_count'] }} · NIK dup {{ $row['duplicate_nik'] ? 'yes' : 'no' }} · Pair dup {{ $row['duplicate_phone_kavling'] ? 'yes' : 'no' }} · Kavling reuse {{ $row['reused_kavling'] ? 'yes' : 'no' }}</td></tr>@endforeach</tbody></table></div>
    </section>
