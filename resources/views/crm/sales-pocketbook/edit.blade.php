...
<x-crm.section id="sales-lead-edit" title="Data Lead" description="Bidang bertanda bintang wajib diisi." class="sales-pocketbook-edit-section">
    ...
    <form method="POST" action="{{ route('sales-leads.update', $lead) }}" x-data="salesCascade(@js($projects->map(fn ($project) => ['id' => (string) $project->id, 'branch_id' => (string) $project->branch_id, 'sales_ids' => $project->assignedUsers->pluck('id')->map(fn ($id) => (string) $id)->all()])), @js($salesUsers->map(fn ($sales) => ['id' => (string) $sales->id, 'project_ids' => $sales->assignedProjects->pluck('id')->map(fn ($id) => (string) $id)->all()])), @js(['branch' => old('branch_id', $lead->branch_id), 'project' => old('project_id', $lead->project_id), 'sales' => old('sales_user_id', $lead->sales_user_id), 'promoOptionsEndpoint' => $promoOptionsEndpoint ?? null, 'leadDate' => old('lead_date', $lead->lead_date->toDateString()), 'promos' => $promos->values(), 'promo' => old('promo_name', $lead->id_promo)]))" @submit="setSubmitting()" class="sales-pocketbook-edit-form" data-conflict-form :aria-busy="submitting">
        ...
        @foreach([['Sumber Lead', 'source', $sources, 'Pilih sumber', $lead->source], ['Kanal Masuk', 'platform', $channels, 'Pilih kanal masuk', $lead->platform], ['Aktivitas Lead', 'campaign_name', $activities, 'Pilih aktivitas lead', $lead->campaign_name]] as [$label, $name, $options, $placeholder, $current])
            @php
                $selected = old($name, $current);
            @endphp
            <x-crm.field :label="$label" :for="'edit-lead-'.$name" required :error="$errors->first($name)">
                <select id="{{ 'edit-lead-'.$name }}" class="sales-input" name="{{ $name }}" required>
                    <option value="">{{ $placeholder }}</option>
                    @if(filled($selected) && !in_array($selected, $options, true))
                        <option value="{{ $selected }}" selected>{{ $selected }} (historis)</option>
                    @endif
        ...
        <x-crm.field label="Status Lead" for="edit-lead-status" required :error="$errors->first('current_status')">
            <select id="edit-lead-status" class="sales-input" name="current_status" required>
                ...
                @foreach(\App\Enums\SalesLeadStatus::cases() as $status)
        ...
        <x-crm.field label="Sales" for="edit-lead-sales" required :error="$errors->first('sales_user_id')">
            <select id="edit-lead-sales" class="sales-input" name="sales_user_id" x-model="sales" required {{ Auth::user()->isSales() ? 'disabled' : '' }} aria-invalid="{{ $errors->has('sales_user_id') ? 'true' : 'false' }}" aria-describedby="{{ $errors->has('sales_user_id') ? 'edit-lead-sales-error' : '' }}">@foreach($salesUsers as $sales)<option value="{{ $sales->id }}" x-show="salesVisible('{{ $sales->id }}')" :disabled="!salesVisible('{{ $sales->id }}')">{{ $sales->name }}</option>@endforeach</select>
            @if(Auth::user()->isSales())
                <input type="hidden" name="sales_user_id" value="{{ $lead->sales_user_id }}">
            @endif
        </x-crm.field>
        <x-crm.field label="Referensi Konsumen Tertaut" for="edit-lead-consumer-reference" :error="$errors->first('linked_consumer_reference')"><input id="edit-lead-consumer-reference" class="sales-input" name="linked_consumer_reference" value="{{ old('linked_consumer_reference', $lead->linked_consumer_reference) }}" aria-invalid="{{ $errors->has('linked_consumer_reference') ? 'true' : 'false' }}" aria-describedby="{{ $errors->has('linked_consumer_reference') ? 'edit-lead-consumer-reference-error' : '' }}"></x-crm.field>
        <x-crm.field label="Catatan" for="edit-lead-notes" :error="$errors->first('notes')" class="md:col-span-2"><textarea id="edit-lead-notes" class="sales-input" name="notes" rows="4" aria-invalid="{{ $errors->has('notes') ? 'true' : 'false' }}" aria-describedby="{{ $errors->has('notes') ? 'edit-lead-notes-error' : '' }}">{{ old('notes', $lead->notes) }}</textarea></x-crm.field>
