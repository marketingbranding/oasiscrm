            ...
            <div x-show="open" x-cloak
                ...
                <x-crm.notif-section :items="$overdueItems" color="#c0392b" label="Terlewat" text-color="text-white">
                    ...
                    <div class="px-3 py-2 text-sm font-['Times_New_Roman']">
                        <div class="font-bold">[{{ strtoupper($item->item_type) }}] {{ $item->title }}</div>
                ...
                <x-crm.notif-section :items="$tomorrowItems" color="#9ab6c8" label="Besok / H-1" text-color="text-black">
                    ...
                    <div class="px-3 py-2 text-sm font-['Times_New_Roman']">
                        <div class="font-bold">[{{ strtoupper($item->item_type) }}] {{ $item->title }}</div>
                        <div class="text-xs text-gray-600">Besok — {{ $item->scheduled_date->format('d M Y') }}</div>
                    </div>
                    @endforeach
                </x-crm.notif-section>
    ...
    <div class="flex flex-col sm:flex-row pt-14">
        <div id="crm-sidebar" :class="[sidebarOpen ? 'block' : 'hidden', sidebarPinned ? 'sm:w-56' : 'sm:w-16 sm:hover:w-56']"
            ...
            <nav :class="sidebarPinned ? 'sm:overflow-y-auto' : 'sm:overflow-y-hidden sm:group-hover:overflow-y-auto'" class="overflow-x-hidden overflow-y-auto flex-1 min-h-0 p-2">
                ...
                @endunless
                @if(Auth::user()?->hasScopedPermission('work_planner'))
                ...
                @endif
                @unless($isSalesNavigation)
                @if(Auth::user()?->hasPermission('database.view'))
                ...
                @endif
                @if(Auth::user()?->hasPermission('consumer_progress.view'))
                ...
                @if(Auth::user()?->hasPermission('bridge_fund.view'))
                ...
                <a href="{{ route('expenses.index') }}" class="flex items-center px-3 py-2 gap-2 text-sm font-[Helvetica] font-bold text-black hover:bg-[#b3bd95] hover:text-black border border-black mb-1 rounded-none {{ request()->routeIs('expenses.*') ? 'bg-[#b3bd95]' : 'bg-white' }}">
                    <span class="inline-block w-4 h-4 shrink-0 text-center">Rp</span><span :class="sidebarPinned ? '' : 'sm:w-0 sm:overflow-hidden sm:whitespace-nowrap'" class="group-hover:sm:w-auto transition-all duration-200 delay-150">Pengeluaran</span>
                </a>
                @if(Auth::user()->hasPermission('expenses.manage_categories'))
                <a href="{{ route('expense-categories.index') }}" class="ml-4 flex items-center px-3 py-1.5 gap-2 text-xs font-[Helvetica] font-bold text-black hover:bg-[#b3bd95] border border-black mb-1 rounded-none {{ request()->routeIs('expense-categories.*') ? 'bg-[#b3bd95]' : 'bg-white' }}">
                ...
                @endif
                @if(Auth::user() && Auth::user()->hasAnyPermission(['branches.manage', 'projects.manage', 'users.view', 'system_health.view']))
                <div class="border-t border-dashed border-gray-400 mx-2 my-2"></div>
                <div class="px-2 mb-0.5 text-[9px] font-[Helvetica] font-bold text-gray-400 uppercase tracking-[0.15em]">
