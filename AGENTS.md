...
### Marison V2 migration

Phase A imports versioned Marison V2.5.50 JSON packages through the Superadmin-only `admin.marison-migrations.*` workflow. The root identity is `marison_v2 + branch_code + id_transaksi_v2`; never identify or match by name, NIK, phone, kavling, row number, or document number alone. Project resolution uses only persisted exact `marison_project_mappings`; unknown branches and missing mappings block preview.

Raw import creates `ConsumerApplication` with `customer_id`, `nama_konsumen`, `nik`, and `kavling_id` null, imports typed process/history facts, preserves unlinked metadata, and creates a `PENDING` `ConsumerMigrationReconciliation`. It never creates a Customer or `ConsumerKavlingAssignment`. `proses_bank` is the only canonical SP3K source; PPJB mirrors never override it. Bank attempt rows merge `bank_attempts`, Pemberkasan, and Proses Bank by `id_bank_attempt`, falling back to transaction + berkas + attempt number. Confirmation imports only `READY` rows atomically; `ALREADY_IMPORTED`, `SOURCE_CHANGED_REVIEW`, and `NEEDS_REVIEW` are retained without overwrite. Phase B Lite provides `admin.marison-migrations.reconciliation.*` queue/detail/apply routes for users with `consumer_progress.manage` and branch/project scope. Admin explicitly selects an existing Customer or creates one by name, then applies `LANJUT`, `MUNDUR`, `PINDAH_KAVLING`, `REJECT`, or `SELESAI` through `ConsumerKavlingLifecycleService`; raw controllers never write assignment rows. No automatic matching, bulk reconciliation, or advanced workflow is included. Phase B owns customer/kavling decisions through `ConsumerKavlingLifecycleService`.
...
`SalesLeadSpreadsheetContract` resolves only `sales_lead.branch_id -> branches.sheet_id` and validates the target tab contract. The verified live `lead` contract uses A:L headers `id_lead,id_promo,tanggal_lead,sumber_lead,kanal_masuk,aktivitas_lead,nama_konsumen,no_hp,proyek,sales_pic,status_lead,keterangan`, blank M, and helper lists N:P. Shipped aliases `sumber`, `platform`, and `campaign` remain accepted while OASIS keeps semantic local fields `source`, `platform`, and `campaign_name`. Metadata is appended after the last occupied helper header. Strict values come from the branch workbook; project and Sales PIC values use branch-scoped exact identities with no fuzzy or cross-branch matching. `SalesLeadSpreadsheetWriter` owns immediate append/update, exact trailing OASIS metadata, formula preservation, strict preflight, and request-supplied UUID-idempotent retry. `SalesLeadLifecycleService` owns web operations; `SalesLeadLifecycleSyncService` owns pull/reconciliation. Keep these responsibilities separate from Database and Konsumen Progress sync.
...
Primary Sales also receives `sales_pocketbook.sync`, limited to an active branch covered by a current active project assignment, own/manage scope, and branch view access; this does not grant Database sync or generic `branch_user.can_sync`. Lifecycle sync status is read-only for scoped branch viewers. Committed runs are `partial_success` when unresolved items or capability/configuration issues remain, and rows marked with remote `oasis_deleted_at` are ignored without deleting audited local leads.
