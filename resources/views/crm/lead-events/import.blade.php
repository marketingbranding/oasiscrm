...
@section('title', 'Import Lead Events - Oasis CRM')

@section('content')
    <div class="bg-[#e6915d] border-2 border-black px-4 py-2 mb-6">
        <h1 class="font-['Arial_Black'] font-black text-xl uppercase">Import Lead Events</h1>
    </div>
    ...
    <div class="bg-white border-2 border-black p-4 mb-6">
        <div class="text-sm font-['Times_New_Roman'] mb-4 leading-relaxed">
            ...
            <p class="mt-2 text-xs text-gray-600">
                Kolom wajib: <strong>Event ID</strong> atau <strong>Proyek</strong>, dan <strong>Tgl Mulai</strong>.
                Baris dengan data tidak valid akan dilewati.
            </p>
        </div>

        <div class="flex items-center gap-3 mb-6">
            <a href="{{ route('lead-events.export-template') }}"
        ...
        <form method="POST" action="{{ route('lead-events.import-store') }}" enctype="multipart/form-data">
            @csrf

            <label class="block text-sm font-[Helvetica] font-bold mb-2">Pilih File XLSX:</label>
            <input type="file" name="file" accept=".xlsx"
                   class="block w-full border-2 border-black px-3 py-2 text-sm font-['Times_New_Roman'] bg-white rounded-none file:border-0 file:bg-[#e6915d] file:text-black file:px-3 file:py-1 file:mr-3 file:font-[Helvetica] file:font-bold file:cursor-pointer">
            ...
            <div class="flex items-center gap-2 mt-4">
                <a href="{{ route('lead-events.index') }}"
