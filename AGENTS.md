...
- Keep primary list actions in the same toolbar row as search and `Filter`, aligned after the filter controls when desktop space allows (for example Export/Import and `+ Tambah`). On smaller screens the toolbar may wrap, but search, filter, and actions must remain grouped in one bordered toolbar instead of being split between the page header and table controls.
- Toolbar structure is shared, but action colors must follow the current module's established accent color. Use the module accent for the primary create/add action, neutral white for secondary actions such as export, and consistent black borders; never copy another module's accent color merely because its toolbar is used as the layout reference.
