...
Recommended direction, not implemented: migrate remaining Google mirror/snapshot modules to normalized local data with explicit import/sync boundaries. Detailed audit, Phase 1 schema foundation, Phase 2 manual TSV paste reconciliation, Phase 3 read comparison, Phase 4 local read foundation, and Phase 5 Dashboard local read foundation: `docs/DATABASE_LOCAL_MIGRATION_PLAN.md` (**DASHBOARD LOCAL READ FOUNDATION IMPLEMENTED / DEFAULT LEGACY / LOCAL MODE OPT-IN / NO WRITE CUTOVER**). Konsumen Progress and Dashboard consumer stage counts use centralized read adapters; `CONSUMER_PROGRESS_READ_SOURCE=local` and `DASHBOARD_CONSUMER_READ_SOURCE=local` are explicit deployment overrides, while local failures fall back to cached legacy data and successful empty local results remain empty. Lead, activity, sync-health, Sales, Dana Talangan, ContentItem, and Database-backed Dashboard widgets remain unchanged. The Superadmin-only `consumer-comparison.index` page remains diagnostic and read-only; Phase 5.5 adds request-time pilot-readiness summary only, with no source switch, repair, or write path. `/consumer-import` now accepts operational spreadsheet headers, encrypts revealable NIK at rest, masks preview NIK, audits explicit Superadmin reveal, and derives completeness without changing legacy import aliases or normal module reads.
