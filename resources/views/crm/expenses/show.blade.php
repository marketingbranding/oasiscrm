...
@section('content')
<div x-data="{ cancelOpen: {{ $errors->has('cancellation_reason') ? 'true' : 'false' }} }">
        ...
        <section class="border-2 border-black bg-white lg:col-span-2">
            ...
            <dl class="grid grid-cols-1 md:grid-cols-2">
                @foreach([
                    ...
                    'Nama Vendor' => $expense->vendor_name ?: '—', 'Metode Pembayaran' => \App\Models\Expense::PAYMENT_METHODS[$expense->payment_method] ?? '—',
    ...
    <div x-show="cancelOpen" x-cloak class="fixed inset-0 z-[900] flex items-center justify-center bg-black/70 px-4" @keydown.escape.window="cancelOpen=false">
        <form method="POST" action="{{ route('expenses.cancel', $expense) }}" class="w-full max-w-lg border-2 border-black bg-white" @click.outside="cancelOpen=false">
            @csrf @method('PATCH')
            ...
            <div class="bg-[#c0392b] px-4 py-2 font-[Helvetica] text-sm font-bold uppercase text-white">Batalkan Pengeluaran</div>
            ...
            <div class="flex gap-2 border-t-2 border-black bg-gray-100 p-4"><button class="border-2 border-black bg-[#c0392b] px-4 py-2 font-bold text-white">Konfirmasi Pembatalan</button><button type="button" @click="cancelOpen=false" class="border-2 border-black bg-white px-4 py-2 font-bold">Kembali</button></div>
        </form>
    </div>
</div>
@endsection
