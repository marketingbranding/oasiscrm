...
<div class="space-y-4" x-data="{ sales: @js($initialSalesId), project: @js($initialProjectId), branch: @js((string) old('branch_id', $initialProjects->count() === 1 ? $initialProjects->first()['branch_id'] : '')), projectsBySales: @js($projectsBySales), get projects() { return this.projectsBySales[this.sales] || [] }, salesChanged() { this.project = this.projects.length === 1 ? this.projects[0].id : ''; this.projectChanged() }, projectChanged() { this.branch = this.projects.find(item => item.id === this.project)?.branch_id || '' } }">
    ...
    <section class="border-2 border-black bg-white"><header class="flex items-center justify-between border-b-2 border-black bg-black px-4 py-2 text-white"><h2 class="text-sm font-bold uppercase">Lead Tim</h2><span>{{ $leads->total() }} lead</span></header><div class="crm-table-scroll"><table class="crm-data-table"><thead><tr><th>Tanggal</th><th>Konsumen</th><th>Sales PIC</th><th>Cabang</th><th>Proyek</th><th>Promo</th><th>Status Lead</th><th>Aksi</th></tr></thead><tbody>@forelse($leads as $lead)<tr><td>{{ $lead->lead_date->format('d/m/Y') }}</td><td>{{ $lead->customer_name }}</td><td>{{ $lead->sales?->name }}</td><td>{{ $lead->branch?->name }}</td><td>{{ $lead->project?->project_name }}</td><td>{{ $lead->id_promo ?: '-' }}</td><td>{{ $lead->current_status->label() }}</td><td><a class="font-bold text-[#0000ee] underline" href="{{ route('sales-leads.edit', $lead) }}">Edit</a></td></tr>@empty<tr><td colspan="8"><x-crm.empty-state title="Belum ada lead tim" description="Lead anggota aktif tim pada periode ini akan muncul di sini." /></td></tr>@endforelse</tbody></table></div><x-crm.pagination :collection="$leads" :show-per-page="false" /></section>
</div>
@endsection
