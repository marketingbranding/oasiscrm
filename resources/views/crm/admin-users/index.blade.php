...
@php
    ...
    $isImpersonating = $impersonation->isActive(request());
    $canBulkResetAccess = auth()->user()->isSuperadmin() && ! $isImpersonating;
...
<div class="border-2 border-black bg-white p-3 mb-4 flex flex-wrap items-center gap-2">
    ...
    @if(! $isImpersonating && auth()->user()->hasAllPermissions(\App\Policies\UserImportBatchPolicy::REQUIRED_PERMISSIONS))<a href="{{ route('admin-users.import') }}" class="border-2 border-black bg-[#8c9ae0] px-4 py-2 text-xs font-bold">IMPORT USER XLSX</a>@endif
    @if(! $isImpersonating) @can('users.create')<a href="{{ route('admin-users.create') }}" class="border-2 border-black bg-[#8c9ae0] px-4 py-2 text-xs font-bold">+ TAMBAH</a>@endcan @endif
</div>
...
<tbody>@forelse($users as $user)<tr>@if($canBulkResetAccess)<td>@if($user->id !== auth()->id())<input type="checkbox" value="{{ $user->id }}" x-model="selected" aria-label="Pilih {{ $user->name }}" class="size-5">@endif</td>@endif<td class="font-bold"><a class="text-[#0000ee] underline" href="{{ route('admin-users.show',$user) }}">{{ $user->name }}</a></td><td>{{ $user->email }}</td><td>{{ $user->role?->name ?? '-' }}</td><td>{{ $user->branch?->name ?? '-' }}</td><td title="{{ $user->assignedProjects->pluck('project_name')->join(', ') }}">{{ $user->assignedProjects->pluck('project_name')->join(', ') ?: '-' }}</td><td>{{ $user->supervisor?->name ?? '-' }}</td><td><span class="border border-black px-2 py-0.5 font-bold">{{ str_replace('_',' ',strtoupper($user->account_status->value)) }}</span></td><td>{{ $user->last_login_at?->format('d/m/Y H:i') ?? '-' }}</td><td class="whitespace-nowrap"><a class="text-[#0000ee] font-bold underline" href="{{ route('admin-users.show',$user) }}">View</a> @if(! $isImpersonating) @can('update',$user) <a class="text-[#0000ee] font-bold underline ml-2" href="{{ route('admin-users.edit',$user) }}">Edit</a>@endcan @if($impersonation->canImpersonate(auth()->user(), $user)) <button type="button" class="ml-2 font-bold text-[#0000ee] underline" @click="impersonationTarget = @js(['name' => $user->name, 'url' => route('admin-users.impersonate', $user)]); $dispatch('oasis:modal-open', { name: 'impersonate-user', trigger: $el })">Masuk sebagai User</button> @endif @endif</td></tr>@empty<tr><td colspan="{{ $canBulkResetAccess ? 10 : 9 }}" class="text-center">Tidak ada pengguna.</td></tr>@endforelse</tbody></table></div>
<div class="mt-4">{{ $users->links() }}</div>

@if(! $isImpersonating)
<x-crm.modal name="impersonate-user" title="Konfirmasi Masuk sebagai User" size="sm">
    <form method="POST" :action="impersonationTarget?.url ?? ''">
        @csrf
        <div class="space-y-3 text-sm">
            <p>Anda akan masuk sebagai <strong x-text="impersonationTarget?.name"></strong>.</p>
            <p>Password pengguna tidak berubah.</p>
            <p>Semua tindakan selama impersonasi akan diaudit.</p>
        </div>
        <div class="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            <x-crm.button type="button" variant="secondary" @click="$dispatch('oasis:modal-close', { name: 'impersonate-user' })">Batal</x-crm.button>
            <x-crm.button type="submit" variant="primary">Masuk sebagai User</x-crm.button>
        </div>
    </form>
</x-crm.modal>
@endif
