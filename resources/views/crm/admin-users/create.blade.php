    ...
    <fieldset class="border-2 border-black">
        ...
        <div class="space-y-3 p-4">
            ...
            <div x-cloak x-show="provisioningMode === 'direct'" class="grid gap-4 border-t-2 border-black pt-4 md:grid-cols-2">
                <div><label for="temporary_password" class="mb-1 block font-[Helvetica] text-xs font-bold uppercase">Password Sementara</label><x-password-input id="temporary_password" name="temporary_password" x-bind:required="provisioningMode === 'direct'" autocomplete="new-password" class="w-full rounded-none border-2 border-black px-3 py-2 text-sm" />@error('temporary_password')<p class="mt-1 text-xs font-bold text-[#e91d2a]">{{ $message }}</p>@enderror</div>
                <div><label for="temporary_password_confirmation" class="mb-1 block font-[Helvetica] text-xs font-bold uppercase">Konfirmasi Password Sementara</label><x-password-input id="temporary_password_confirmation" name="temporary_password_confirmation" x-bind:required="provisioningMode === 'direct'" autocomplete="new-password" class="w-full rounded-none border-2 border-black px-3 py-2 text-sm" /></div>
            </div>
            @endif
        </div>
    </fieldset>
